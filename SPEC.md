...
2. **Outline panel** (`src/outline/outline.ts`, `src/ui/OutlinePanel.tsx`): the sheet's own headings, numbered I / A / 1 for show only. Add, rename (double-click, Enter for the next, Tab to indent), move a section with its writing, and take one out — anything written in it goes to Cuts. It shares the right-hand column with Notes, and on iPhone it takes the editor's place like Notes does. Because it edits the headings in the sheet, there is no second document to keep in step.
