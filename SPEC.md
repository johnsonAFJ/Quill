...
4. **Folding:** an arrow beside each heading (except the sheet's title on its first line) folds its section away; remembered per device. ⌥⌘[ / ⌥⌘] fold and open the section the cursor is anywhere in (`sectionAt`, `foldSection`, `unfoldSection`), matched by physical key because ⌥ turns [ and ] into “ and ‘ on a Mac.
...
### Phase 6: Templates, outlines and banned words (built 2026-09-25)
1. **Templates:** sheets in a top-level `Templates` group (`src/templates/templates.ts`). "New sheet from template…" in a group's menu; "Save as template…" in a sheet's menu writes one, making the group if needed. `fillIn` swaps `{{title}}`, `{{date}}`, `{{day}}` and `{{time}}`; a template whose title line is left empty starts the cursor there.
...
3. **Banned words** (`src/text/banned.ts`, `src/editor/banned.ts`): a nudge, never a block. `_Banned words.md` holds the list (hidden from the Library, prose lines in it are ignored so notes to self are safe); matches get a wavy underline and a tally in the strip under the writing (`.editor-foot`, in the flow so it never covers a line; it holds the word count on the right and shows only when there's something in it). Off unless switched on, per device. A strict mode that refuses the word was considered and dropped: fighting autocorrect mid-word is worse than the habit.

### Phase 7: Writing log (built 2026-09-25)
A **Writing log** row in the Library opens a month calendar (`src/log/log.ts`, `src/ui/WritingLogPane.tsx`), arrows to earlier months, today outlined, future days dimmed. Days shade in four steps by words added (150 / 500 / 1,200). A day's card shows words written, words cut, sprints and the sheets worked in (kept by the sheet's lasting id, with its latest name). Totals line: words since the first day, days written, longest day. Counting happens only in the editor's own change handler, as the word-count difference per edit: positive adds to "written", negative to "cut". Each device writes its own `_quill/log/<device id>.json` (debounced 8 seconds, and at once when the app is hidden); the calendar merges every device's file. No streaks, goals or reminders, by design. The test copy exposes `window.quill.engine` for checking things by hand (`import.meta.env.DEV` only).
...
### Ongoing
- Seasonal themes: **Halloween built 2026-09-25**, announced by a release note that waits in `CHANGELOG.md` for its own season (see below), so no "What's new" pop-up gives it away early. Its start date was drawn at random between October 1 and 15 at Alex's request and is written only in `SEASONS` (`src/app/season.ts`), never here, in the changelog or in chat, so it surprises him; it ends November 1. A preview ran until noon Eastern on 2026-09-26 (`PREVIEW_UNTIL`); after that "Show me now" disappears from Settings and behaves like "When it's time", so the look can't be summoned early. Settings → Seasonal look: "When it's time" (the default), "Off", and "Not this year" while one is showing. Most of it lands on the frame: deep aubergine sidebars, a still candlelight glow at the top of each, a cobweb masked into the Library's top corner, pumpkin accents and a small 🎃 beside "Library". The writing area only warms a shade (a hint of candle in the paper), which bends the original "never the writing area" rule on purpose, for the mood; nothing anywhere moves, flickers or animates. A **line a day** sits at the foot of the Library above Trash, from books out of copyright (Poe, Stoker, Shelley, Shakespeare, Stevenson, Irving, Le Fanu, James, Dickens, Lovecraft, Chambers, Gilman), short and credited; the day's date picks it, so every device shows the same line (`QUOTES` and `quoteOfDay` in `src/app/season.ts`). Later seasons add their own list under their id. More seasons go in the same list.

  Its release note is already in `CHANGELOG.md`, marked `<!-- with season: halloween -->` on its heading: `parseChangelog` reads the marker and `readyUpdates` keeps that update out of "What's new" until the season is actually showing by the calendar (a preview doesn't count, so looking early doesn't spoil it). Nothing has to be published by hand on the day.
- More PDF styles, added one at a time. **Ghost Story** added 2026-09-25: IM Fell English (OFL, bundled), small-caps title and headings, a three-line drop capital (`markDropCap`), justified text, `— n —` page numbers.
