...
### Ongoing
- Seasonal themes: **Halloween built 2026-09-25**, deliberately left out of `CHANGELOG.md` until release day so no "What's new" pop-up gives it away. Its start date was drawn at random between October 1 and 15 at Alex's request and is written only in `SEASONS` (`src/app/season.ts`), never here, in the changelog or in chat, so it surprises him; it ends November 1. A preview ran until noon Eastern on 2026-09-26 (`PREVIEW_UNTIL`); after that "Show me now" disappears from Settings and behaves like "When it's time", so the look can't be summoned early. Settings → Seasonal look: "When it's time" (the default), "Off", and "Not this year" while one is showing. Most of it lands on the frame: deep aubergine sidebars, a still candlelight glow at the top of each, a cobweb masked into the Library's top corner, pumpkin accents and a small 🎃 beside "Library". The writing area only warms a shade (a hint of candle in the paper), which bends the original "never the writing area" rule on purpose, for the mood; nothing anywhere moves, flickers or animates. A **line a day** sits at the foot of the Library above Trash, from books out of copyright (Poe, Stoker, Shelley, Shakespeare, Stevenson, Irving, Le Fanu, James, Dickens, Lovecraft, Chambers, Gilman), short and credited; the day's date picks it, so every device shows the same line (`QUOTES` and `quoteOfDay` in `src/app/season.ts`). Later seasons add their own list under their id. More seasons go in the same list.

  Release day (the morning it appears): paste this at the top of `CHANGELOG.md` with that day's date.

  ```
  ## Oct <the day> · 🎃 Quill puts on a costume

  - 🕯️ **Quill is dressed for Halloween.** The Library and the sheet list have gone candle-lit and pumpkin-dark for the rest of October, and a line a day from an old spooky book waits at the foot of the Library. Your writing area is untouched: same paper, same ink, not a cobweb on your words.
